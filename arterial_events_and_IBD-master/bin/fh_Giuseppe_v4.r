...
list.of.packages <- c("easypackages", "PRROC", "e1071", "pacman","readr",
                      "randomForest","caret","e1071","mltools","boot",
                      "umap","kernlab","xgboost","MLmetrics")
...
cat("RF 1000x MonteCarlo, arterial event, Random Forests: 
")
...
cat("1000x MonteCarlo, event type, linear SVM: 
")
m_std(PREDICTIONS,ACTUALS)
#measure  mean    sd
#     MCC 0.112 0.452
...
## xgboost on the Arterial_Event - 1000 runs x  MonteCarlo stratified splits
PREDICTIONS <- ACTUALS <- list()
for(j in 1:N){
  TS_O <- sample(ae_cl_one)[1:n_ae_ts_one]
  TS_Z <- sample(ae_cl_zero)[1:n_ae_ts_zero]
  ...
  train.data <- as.matrix(arterial_event[TR,1:n_preds])
  train.label = as.integer(arterial_event$TARGET_arterial_event[TR])-1
  test.data <- as.matrix(arterial_event[TS,1:n_preds])
  test.label <- as.integer(arterial_event$TARGET_arterial_event[TS])-1
  ...
  PREDICTIONS[[j]] <- floor(xgb.pred+0.5)
  ACTUALS[[j]] <- test.label
}
cat("1000x MonteCarlo, arterial_event, xgboost
")
m_std(PREDICTIONS,ACTUALS)
#measure  mean    sd
#     MCC 0.401 0.206
...
## xgboost on the Event_Type - 1000 runs x  MonteCarlo stratified splits
PREDICTIONS <- ACTUALS <- list()
for(j in 1:N){
  TS_O <- sample(et_cl_one)[1:n_et_ts_one]
  TS_Z <- sample(et_cl_zero)[1:n_et_ts_zero]
  ...
  train.data <- as.matrix(event_type[TR,1:n_preds])
  train.label = as.integer(event_type$TARGET_type_0ACS_1stroke[TR])-1
  test.data <- as.matrix(event_type[TS,1:n_preds])
  test.label <- as.integer(event_type$TARGET_type_0ACS_1stroke[TS])-1
  xgb.train = xgb.DMatrix(data=train.data,label=train.label)
  xgb.test = xgb.DMatrix(data=test.data,label=test.label)
  params = list(
    booster="gblinear",
...
cat("1000x MonteCarlo, event_type, xgboost
")
m_std(PREDICTIONS,ACTUALS)
#measure  mean    sd
#     MCC 0.233 0.504
...
### Logit on Arterial Event, 1000x MonteCarlo
PREDICTIONS <- ACTUALS <- list()
for(j in 1:N){
  TS_O <- sample(ae_cl_one)[1:n_ae_ts_one]
  TS_Z <- sample(ae_cl_zero)[1:n_ae_ts_zero]
  ...
  out <- glm(TARGET_arterial_event ~. ,data = arterial_event[TR,], family = binomial)
  pp <- predict(out,arterial_event[TS,1:11],type="response")
  
  PREDICTIONS[[j]] <- floor(pp+0.5)
  ACTUALS[[j]] <- as.integer(arterial_event$TARGET_arterial_event[TS])-1
}
cat("MonteCarlo, arterial_event, logit
")
m_std(PREDICTIONS,ACTUALS)
#     MCC 0.340 0.230
#      F1 0.797 0.073
...
### Logit on  Event Type, 1000x MonteCarlo
PREDICTIONS <- ACTUALS <- list()
for(j in 1:N){
  TS_O <- sample(et_cl_one)[1:n_et_ts_one]
  TS_Z <- sample(et_cl_zero)[1:n_et_ts_zero]
  ...
  TR <- sample(c(TR_O,TR_Z))
  TS <- sample(c(TS_O,TS_Z))
  out <- glm(TARGET_type_0ACS_1stroke ~. ,data = event_type[TR,], family = binomial)
  pp <- predict(out,event_type[TS,1:11],type="response")
  
  PREDICTIONS[[j]] <- floor(pp+0.5)
  ACTUALS[[j]] <- as.integer(event_type$TARGET_type_0ACS_1stroke[TS])-1
}
cat("MonteCarlo, event_type, logit
")
m_std(PREDICTIONS,ACTUALS)
#     MCC 0.058 0.441
#      F1 0.715 0.176
